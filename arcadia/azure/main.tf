resource "azurerm_public_ip" "ip" {
  ...
  name                = format("%s-%s-public-ip", each.value.name, var.app)
...
resource "azurerm_network_interface" "network_interface" {
  ...
  name                = format("%s-%s", var.azure_network_interfaces[count.index].name, var.app)
  location            = var.azure_region
  resource_group_name = var.azure_resource_group_name
  tags                = var.azure_network_interfaces[count.index].tags
  ip_configuration {
    name                          = format("%s-%s-ipcfg", var.azure_network_interfaces[count.index].name, var.app)
...
resource "azurerm_linux_virtual_machine" "vm" {
  ...
  tags        = var.custom_tags
  custom_data = base64encode(templatefile("./templates/ubuntu_custom_data.tftpl", { 
    appdir = var.appdir
    app = var.app
  }))
}
